	...
	$('#menuCatalogTree').tree({
		...
		onCheck:function(node,checked){
			if(checked){
				...
				var loadurl= '${ctx_admin}/security/sysUserpermission/searchMenuPrivilege?menuId='+node.id+"&sysUserId="+sysUserId+"&random="+random;
    ...
    <div  data-options="region:'center'" class="no-border">
        <div class="easyui-layout" data-options="fit:true">
            ...
            <div data-options="region:'south'" class="buttons">
				...
				<input type="button" class="button" id="saveMenuBtn" value="保 存"/> 
            </div>
        </div>
    </div>
</div>
...
$('#saveMenuPrivileg').ajaxForm({
    url:"${ctx_admin}/security/sysUserpermission/saveMenuPrivilege.json",
...
$("#saveMenuBtn").click(function(){
	if($("#sysUserId").val()==null||$("#sysUserId").val().length==0){
		$.messager.alert('用户授权操作','请先从第一步选择员工！');
		return;
	}
