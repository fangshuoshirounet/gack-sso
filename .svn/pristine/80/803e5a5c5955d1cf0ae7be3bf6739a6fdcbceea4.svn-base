	...
	$('#menuCatalogTree').tree({
		...
		onCheck:function(node,checked){
			if(checked){
				...
				var loadurl= '${ctx_admin}/security/rolepermission/searchmenu?menuId='+node.id+"&roleId="+roleId+"&random="+random;
        ...
        <div class="easyui-layout" data-options="fit:true">
            ...
            <div id="flexgrid" data-options="region:'center'" class="content">
                <form id="menuPermissionForm" method="post">
					...
					<input type="hidden" name="roleCode" id="roleCode" value="${model.roleCode }"/> 
				</form>
            </div>
            <div data-options="region:'south'" class="buttons">
            	<input type="button" class="button" id="menuBackBtn" value="取消"/>
...
$("#menuBackBtn").click(function(){
	window.location.href="${ctx_admin}/security/rolepermission?&type=menu";
});

 
$('#menuPermissionForm').ajaxForm({
    url:"${ctx_admin}/security/applyrolepermission/saveMenuOperatePermission",
