...
(function($) {
	$.addFlex = function(t, p){
		...
		p = $.extend({
					...
					errormsg : 'System Exception',// 错误提示信息
					usepager : true, // 是否分页
					nowrap : true, // 是否不换行
					page : 1, // 默认当前页
					total : 0, // 总条目数
					...
					title:"Query List",//默认显示
					showTableToggleBtn:true,//是否显示表格的收缩按钮
					titleAlign : 'center',// 控制标题的align
					pagestat : 'display {from} to {to} , total {total} record',// 显示当前页和总页面的样式
					pagetext : 'currpage',
					outof : '  total',
					findtext : 'query',
					procmsg : 'Being queried, please wait a moment...',
					query : '',
					qtype : '',
					nomsg : 'No eligible records exist!',
...
//			$(g.tDiv).append("<h1 class=\"roundTitle\">搜索结果</h1>");// add by wangqiang
		...
		if (p.usepager) {
			...
			var html = ''
			if (p.showpager) {
				html += ' <div class="pGroup"> <div class="pFirst pButton" title="fist page"><span></span></div><div class="pPrev pButton" title="previous page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'
						+ p.pagetext
						+ ' <input type="text" size="2" value="1" /> '
						+ p.outof
						+ ' <span> 1 </span></span><span class="pcontrol"> page </span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton" title="下一页"><span></span></div><div class="pLast pButton" title="last page"><span></span></div> </div><div class="pGroup"><div ><span class="pPageStat"></span></div></div><div class="btnseparator"></div> ';
			...
			html += '<div class="pGroup" title="reflesh" style="margin-right:5px;"> <div class="pReload pButton"><span></span></div> </div><div class="btnseparator"></div>';
			if (p.toExcel) {
				html += '<div class="pGroup" title="Export as Excel" style="margin-rigth:5px;"> <div class="pExcel pButton"><span></span></div> </div><div class="btnseparator"></div>';
			}
			if (p.print) {
				html += '<div class="pGroup" title="Print" style="margin-right:5px;right:5px;"> <div class="pPrint pButton"><span></span></div> </div><div class="btnseparator"></div>';
			}
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div class="ptogtitle" title="Pack up/on"><span></span></div>');
		...
		if ($('th', g.hDiv).length) {
			...
			$(g.nBtn).addClass('nBtn').html('<div></div>').attr('title',
					'Show/hide columns').click(function() {
						$(g.nDiv).toggle();
						return true;
					});
