	...
	<div data-options="region:'center'" class="no-border">
		<div class="easyui-layout" data-options="fit:true">
					...
					<form id="serachForm">
						...
						<table class="search-table">
							<tr>
								...
								<td class="search-buttons"><shiro:hasPermission name="security:applyuserrole:pending:query">
			...
			<div data-options="region:'south'" class="buttons">
				<shiro:hasPermission name="security:applyuserrole:pending:view">
					<input type="button" class="button" id="showBtn" value="查看" />
				</shiro:hasPermission>
				<shiro:hasPermission name="security:applyuserrole:pending:delete">
	...
	$(function() { 
				...
				.flexigrid(
						{
							...
							colModel : [  
									...
									},
									{
										...
										operateColumn : [
												{
													html : '<shiro:hasPermission name="security:applyuserrole:pending:view"><a  name="showLink" href="#">查看</a></shiro:hasPermission>'
												},
												{
													html : '<shiro:hasPermission name="security:applyuserrole:pending:delete"><a name="delLink" href="#">取消</a></shiro:hasPermission>'
												} ]
									} ]
						});
		//查询
		...
		$("#showBtn,[name='showLink']").live("click", function(event, data) {
			if (isSelected({
				...
				var url = '${ctx_admin}/security/applyuserrole/' + key;
				top.openDialog({
					href : url,
					resizable : false,
					height : 430,
				...
				function(event, data) {
					...
					if (isSelected({
						...
						$.messager.confirm('取消提示', '您是否确定取消选中的申请?', function(r) {
							if (r) {
								var url = "${ctx_admin}/security/applyuserrole/" + keys
										+ "?_method=DELETE";
								$.ajax({
									type : 'post',
									url : url,
